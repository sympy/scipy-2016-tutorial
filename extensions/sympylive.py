...
def builder_inited(app):
    if not app.config.sympylive_url:
        raise ExtensionError('sympylive_url config value must be set for the sympylive extension to work')

    app.add_javascript(app.config.sympylive_url + '/static/external/ext-core.js')
